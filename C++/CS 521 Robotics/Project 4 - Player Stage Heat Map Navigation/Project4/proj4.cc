...
void Pilot(PlayerClient &robot, Position2dProxy &pp, RangerProxy &rp, Waypoint wp)
{
   cout << "Pilot Starting\n";
...
int main(int argc, char *argv[])
{
  ...
  start.x = pp.GetXPos();
  start.y = pp.GetYPos();
  cout << "Robot Location: " << start.x << ", " << start.y << endl;

  Waypoint target;
  target.x = atof(argv[1]);
  target.y = atof(argv[2]);
  cout << "Robot Target: " << argv[1] << ", " << argv[2] << endl;  
  
//  Pilot(robot, target);
  Map map("hospital_section.pnm", 2.715);
//  outputMap(map, "test.pnm");
  vector<Waypoint> results;

  Navigate(map, start, target, results, 6);

  cout << "Next Waypoints: " << endl;
  for(int j = 0; j < results.size(); j++)
      cout << (j+1) << " - " << results[j].x << ", " << results[j].y << endl;

  for(int i = 0; i < results.size(); i++)
  {
    Pilot(robot, pp,rp, results[i]);
    pp.SetSpeed(0, 0);
    cout << "Waypoint " << (i+1) << " reached." << endl;
    cout << "Next Waypoints: " << endl;
    for(int j = 0; j < results.size(); j++)
	if(j == i+1)
           cout << "(" << (j+1) << ")" << " - " << results[j].x << ", " << results[j].y << endl;
